# Modified from official services module due to problems with state directory
{
  config,
  lib,
  pkgs,
...
let
  cfg = config.custom.services.technitium-dns-server;
...
in
{
  options.custom.services.technitium-dns-server = {
    ...
    runUser = mkOption {
      type = types.str;
      default = "root";
      description = "User to run the technitium-dns-server";
    };

    runGroup = mkOption {
      type = types.str;
      default = "root";
      description = "Group for file permissions for the server files";
    };
  ...
  config = mkIf cfg.enable {
    systemd.tmpfiles.settings."10-technitium".${stateDir}.d = {
      user = cfg.runUser;
      group = cfg.runGroup;
      mode = "0700";
    };
